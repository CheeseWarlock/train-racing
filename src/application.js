...
(function() {
  ...
  Crafty.c("Train", {
    ...
    _moveCurved: function() {
      var angularDiff, counterClockwise, ref;
      counterClockwise = ((ref = this.sourceDirection + this.targetDirection) === "en" || ref === "nw" || ref === "ws" || ref === "se" ? -1 : 1);
  ...
  Crafty.c("PlayerTrain", {
    ...
    _updateCurrentTrack: function(dir) {
      var backThroughCurve, curve, f, isCurving, j, k, len, len1, ref, ref1, straight;
  ...
  Crafty.c("AITrain", {
    ...
    _getSegmentPriority: function(depth, x, y, dir, distance) {
      var cdir, curvedPriority, dropoffPlayer, j, len, newx, newy, otherDist, priority, ref, results, sdir, searchPlayer, station, straightPriority;
    ...
    _updateCurrentTrack: function(dir) {
      var backThroughCurve, curve, curvedPriority, decision, f, isCurving, j, k, len, len1, ref, ref1, straight, straightPriority;
      backThroughCurve = this.reversing && !(this._hasCurveOption() && this._hasStraightOption()) && this.currentTrack.dir.length === 3;
      if (backThroughCurve) {
        ref = this.followers;
        for (j = 0, len = ref.length; j < len; j++) {
  ...
  Crafty.scene('Title', function() {
    ...
    return Crafty.e('TitleText').attr({
      y: 528
    }).text('Game: <a href="https://twitter.com/CheeseWarlock" target="_blank">@CheeseWarlock</a> - Music: <a href="https://twitter.com/FuzzyProxy" target="_blank">@FuzzyProxy</a> (<a href="https://soundcloud.com/fuzzy-proxy/sets/morning-commute-official-soundtrack" target="_blank">Album</a>)');
  });
  ...
  window.Util = {
    ...
    createTrain: function(x, y, playerOne, dir, cars) {
      var front, i, j, k, len, letter, ref, ref1, results1, temp, train;
      if (cars == null) {
        cars = 3;
      }
      ...
      ref1 = (cars > 1 ? (function() {
        results1 = [];
        for (var k = 2, ref = Math.max(2, cars); 2 <= ref ? k <= ref : k >= ref; 2 <= ref ? k++ : k--){ results1.push(k); }
        return results1;
      }).apply(this) : []);
    ...
    setupFromTiled: function(tiledmap) {
      var actuallySwapColours, firstTrain, j, k, l, len, len1, len2, len3, m, prop, ref, ref1, ref2, ref3, secondTrain, tile, tilecode;
      actuallySwapColours = window.SwapColours && Math.random() > 0.5;
      ref = tiledmap.getEntitiesInLayer('Tracks');
      for (j = 0, len = ref.length; j < len; j++) {
        tile = ref[j];
      ...
      ref1 = tiledmap.getEntitiesInLayer('Stations');
      for (k = 0, len1 = ref1.length; k < len1; k++) {
        tile = ref1[k];
        for (prop in tile.__c) {
          if (prop.substring(0, 4) === 'Tile') {
      ...
      ref2 = tiledmap.getEntitiesInLayer('Trains');
      ...
      if (tiledmap.getEntitiesInLayer('Props')) {
        ref3 = tiledmap.getEntitiesInLayer('Props');
  ...
  window.AI = {
    checkAlongSegment: function(x, y, dir) {
      var dist, heading, j, len, stations, track, trainPosition, trainPositions, trainPresences;
      trainPositions = [];
      stations = [];
      trainPresences = {
        playerOne: false,
      ...
      while (track.dir.length !== 3 || track.dir[1] !== Util.opposite(heading)) {
        ...
        x += Util.dirx(heading);
        y += Util.diry(heading);
        for (j = 0, len = trainPositions.length; j < len; j++) {
          trainPosition = trainPositions[j];
  ...
  window.BGMManager = {
    ...
    playNext: function() {
      ...
      this.stop();
      if (!Crafty.audio.muted) {
        this.songIndex = ++this.songIndex % 3;
