...
MockModule = {
	...
	createMockEntities: function(){
		...
		for(var layer = 0; layer < this._source.layers.length; layer++){
			if (this._source.layers[layer].name == "Props" && window.HEADLESS_MODE) {
				continue;
			}
	...
	*/
	...
	createMockEntity:function( layer, dataIndex){			
		...
		var mock = {head:"2D," + (this._renderMethod?this._renderMethod+",":"") + "Tile" + layer.data[dataIndex] + "," + layer.name};	
